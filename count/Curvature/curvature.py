...
def parameter_estimation(seg, der, mode = 0):
    ...
    if(mode == 0):
        A = np.add(np.multiply(der[:,0],seg[:,0]), \
                   np.multiply(der[:,1],seg[:,1]))
        ...
        r = np.mean(np.sqrt(np.square(np.subtract(seg[:,0],x_c)) + \
                            np.square(np.subtract(seg[:,1],y_c))))
        distance =  np.mean(np.square(np.sqrt(np.square(seg[:,0]-x_c) + \
                                              np.square(seg[:,1]-y_c))-r))
...
def clustering(seg,der,x,y):
    ...
    while(i<len(seg)):
        j = i+1
        while(j<len(seg)):
            if (seg[i].size < MAX_NUMBER_THRE or \
                arc_distance(seg[i],seg[j],der[i],der[j],x,y) < MAX_DISTANCE):
...
def find_circles(s, d, plot_show = 0):
    ...
    for i in range(0,l):
        ...
        if(distance<DIS_THRE and r <= MAX_R and \
           r >= MIN_R and percentage >= PER_THRE):
            if(len(circles)>0):
...
def perimeter_exaction(image, original_image, image_filename = 'test.jpg', \
    ...
    for n, contour in enumerate(contours):
        ...
        if(c_length >= MIN_PER_LENGTH and c_length <= MAX_PER_LENGTH):
            ...
            dfs = np.true_divide(dfai,np.sqrt(der[:,1]*der[:,1] + \
                                 der[:,0]*der[:,0]) + np.spacing(1))
            ...
            if(c_index.size > 1):                
                tmpx = x[np.append(np.arange(c_index[c_index.size-1],x.size), \
                               np.arange(0,c_index[0]))]    
                tmpy = y[np.append(np.arange(c_index[c_index.size-1],x.size), \
                               np.arange(0,c_index[0]))]
                tmp_der = der[np.append(np.arange(c_index[c_index.size-1], \
                               x.size), np.arange(0,c_index[0])),:]
            ...
            circle_parameters = find_circles(seg, seg_der)
            
            if(len(perimeter) == 0):
                perimeter = circle_parameters
            elif (len(circle_parameters)>0):
                perimeter = np.vstack((perimeter, circle_parameters))
            ...
            if (plot_show == 1):
                ...
                y_max = max(contour[:,0])
                y_min = min(contour[:,0])
                y_limit = original_image.shape[0]
                x_limit = original_image.shape[1]
                if(x_min-10 >= 0):
                    x_min = x_min-10
                if(x_max+10 <= x_limit):
                    x_max = x_max+10
                ...
                ax.plot(contour[:, 1]-x_min+1, contour[:, 0]-y_min+1, \
                        linewidth=2,color='red')
                ax.set_title('Original Image Edge')
                bx.plot(x[0], y[0], 'x', x[5], y[5], 'o', x, y, \
