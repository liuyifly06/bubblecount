...
omega_X=1./(sigma_X.^2+eps);
omega_Y=1./(sigma_Y.^2+eps);
...
for i=1:N_itermax
    W=omega_X.*omega_Y./(omega_X+b^2*omega_Y-2*b*r.*alpha+eps);

    X_bar=sum(W.*X)/(sum(W)+eps);
    Y_bar=sum(W.*Y)/(sum(W)+eps);

    U=X-X_bar;
    V=Y-Y_bar;

    beta=W.*(U./(omega_Y+eps)+b*V./(omega_X+eps)-(b*U+V).*r./(alpha+eps));

    b=sum(W.*beta.*V)/(sum(W.*beta.*U)+eps);
    b_save(i+1)=b;
    if abs((b_save(i+1)-b_save(i))/(b_save(i+1)+eps))<tol
        break
    end
end
...
sigma_b=sqrt(1/(sum(W.*u.^2)+eps));
sigma_a=sqrt(1./(sum(W)+eps)+x_bar^2*sigma_b^2);